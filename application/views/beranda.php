...
<body>
				...
				<div class="row">
					...
					<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
						<div class="panel panel-default card-view">
							...
							<a href="<?php echo site_url('beranda/tanda_tangan'); ?>"class="btn btn-info btn-sm btn-block">Lihat Data Tanda Tangan</a>
							<br>
							<br>
							<div class="mb-10">
							</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form>
						...
						<div class="form-group">
							<label class="control-label mb-10" for="edit_kesiapa">Kesiapa</label>
							<input type="text" class="form-control" name="edit_kesiapa" id="edit_kesiapa" placeholder="Masukkan Kesiapa">
						</div>
						<div class="form-group">
							<label class="control-label mb-10" for="edit_asalinstansi">Asal Instansi</label>
							<input type="text" class="form-control" name="edit_asalinstansi" id="edit_asalinstansi" placeholder="Masukkan Asal Instansi">
						</div>
						<div class="form-group">
							<label class="control-label mb-10" for="edit_keperluan">keperluan</label>
				...
				<div class="modal-footer">
					<button id="btn-edit" type="button" class="btn btn-sm btn-primary">edit</button>
				</div>
			</div>
		</div>
	</div>
	...
	<script>
		...
		$(document).ready(function () {
			...
			// Event saat modal Edit ditampilkan
			$('#form-edit').on('show.bs.modal', function(event) {
				...
				var kesiapa = button.data('kesiapa');
				var keperluan = button.data('keperluan');
				var asalinstansi = button.data('asalinstansi');

				// Mengisi nilai-nilai input fields pada form Edit
				$('#edit_idtamu').val(id);
				$('#edit_nama').val(nama);
				$('#edit_jeniskelamin').val(jk);
				$('#edit_alamat').val(alamat);
				$('#edit_kesiapa').val(kesiapa);
				$('#edit_keperluan').val(keperluan);
				$('#edit_asalinstansi').val(asalinstansi);
			});
			...
			$('#btn-tambah').click(function () {
				var arg = {
					resultFunction: function (result) {
						...
						// document.getElementById("input_keperluan").innerHTML = array_tamu[3];
			...
			$('#btn-hapus').click(function () {
				...
				$('#btn-edit').click(function() {
					...
					// Mengirim data ke controller untuk perubahan
					$.ajax({
						url: 'beranda/edit/' + id,
						type: 'POST',
						...
						dataType: 'json',
						beforeSend: function() {
							// Tindakan sebelum mengirim data
						},
						success: function(response) {
							if (response.status == 'sukses') {
								// Tampilkan Tabel dan pesan sukses
								tampil_tamu();
								jumlah_tamu();
								swal({
									title: "Sukses",
								...
								// Tampilkan Tabel dan pesan gagal
								tampil_tamu();
								jumlah_tamu();
								swal({
									title: "Gagal",
			...
			$('#form-tambah').on('hidden.bs.modal', function (e) { // Ketika Modal Dialog di Close / tertutup
				$('#form-tambah #nama, #form-tambah #alamat, #form-tambah #keperluan').val('') // Clear inputan menjadi kosong
			})
		})
